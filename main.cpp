...
int main() {
    ...
    size_t walltext_cnt;                // number of different textures in image
    ...
    for(size_t j = 0; j < map_h; ++j) {
        for(size_t i = 0; i < map_w; ++i) {
            ...
            draw_rectangle(framebuffer, win_w, win_h, rect_x, rect_y, rect_w, rect_h, walltext[textid * walltext_size]);
        }
    }
    ...
    for(size_t i = 0; i < win_w / 2; ++i) {
        ...
        for(float t = 0; t < 20; t += 0.01) {
            ...
            if(map[int(cx) + int(cy) * map_w] != ' ') {
                ...
                draw_rectangle(framebuffer, win_w, win_h, win_w / 2 + i, win_h / 2 - column_height / 2, 1, column_height, walltext[textid * walltext_size]);
                break;
            }
        }
    }
